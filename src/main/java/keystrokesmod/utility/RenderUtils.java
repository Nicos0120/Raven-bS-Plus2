...
public class RenderUtils {
    public static Vec3 to2D(double x, double y, double z) {
        ...
        if (result) {
            ...
            return new Vec3(
                    ...
                    (Minecraft.getMinecraft().displayHeight - screenCoords.get(1)) / scaleFactor,
                    screenCoords.get(2)
            );
        }
        return null;
    ...
    private static void multiplyMatrixVector(FloatBuffer matrix, float[] in, float[] out) {
        for (int i = 0; i < 4; i++) {
            out[i] =
                    matrix.get(0 * 4 + i) * in[0] +
                            matrix.get(1 * 4 + i) * in[1] +
                            matrix.get(2 * 4 + i) * in[2] +
                            matrix.get(3 * 4 + i) * in[3];
        }
    }
    ...
    public static void disable3DRender() {
    ...
    // 在 RenderUtils 类中添加
    public static void drawRoundedBorder(float x, float y, float x2, float y2, float radius, int color, float lineWidth) {
    ...
    public static void drawRoundedRectOutline(float x, float y, float x2, float y2, float radius, float lineWidth, int color) {
        ...
        for (int i = 0; i <= 90; i += 3) {
            ...
            GL11.glVertex2d(x + radius + Math.sin(angle) * radius * -1, y + radius + Math.cos(angle) * radius * -1);
        }
        for (int i = 90; i <= 180; i += 3) {
            float angle = (float) Math.toRadians(i);
            GL11.glVertex2d(x + radius + Math.sin(angle) * radius * -1, y2 - radius + Math.cos(angle) * radius * -1);
        }
